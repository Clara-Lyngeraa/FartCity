...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Find (pat, str) ->
      if pat = "" || str = "" then
        Str "Error, tom streng"
      else
        let rec aux (i: int) =
          match str.IndexOf(pat, i) with
          | -1 -> econt ()
          | index ->
              cont (Int index) (fun () -> aux (index+1))
        aux 0 
    | Fail -> econt ()
...
let iconMySolution2 = Every(Write(Prim("*", FromTo(1,4),
  And(Write (CstS "\n"),FromTo(1,4)))));

let str = "Hi there - if there are anyone";
let iconMySolution3 =  Every(Write(Find("there",str)));

let str1 = "Hi there - there is somthing there and i need to find their";
let iconMySolution31 =  Every(Write(Find("there",str1)));
let str2 = ""
let iconMySolution32 =  Every(Write(Find("there",str2)));
let str3 = "I need to find i and i want to know i can find it";
let iconMySolution33 =  Every(Write(Find("i",str3)));

let str4 = "Hi there - if there are anyone";
