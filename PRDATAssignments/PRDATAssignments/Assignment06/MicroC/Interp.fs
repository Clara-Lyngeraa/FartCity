...
and eval e locEnv gloEnv store : int * store = 
  match e with
    ...
    | PreDec(acc) -> let (loc, store1) = access acc locEnv gloEnv store //Fart city exercise 7.4
                     let value = getSto store1 loc
                     let newStore = setSto store1 loc (value - 1)
                     (getSto newStore loc, newStore)
    | PreInc(acc) -> let (loc, store1) = access acc locEnv gloEnv store //Fart city exercise 7.4
                     let value = getSto store1 loc
                     let newStore = setSto store1 loc (value + 1)
                     (getSto newStore loc, newStore)
