...
let rec eval e (env : (string * int) list) : int =
    ...
    | Let(lst, exp) ->
      match lst with
      |(s, ex) :: rest ->
          let evanExp = eval ex env
          eval (Let(rest, exp)) ((s,evanExp) :: env) 
      |[] -> eval exp env
...
let rec freevars e : string list =
    ...
    | Let(lst, exp) ->
          match lst with
          |(s, ex) :: rest -> union (freevars ex, minus (freevars (Let(rest, exp)), [s]))
          |[] -> freevars exp
    | Prim(ope, e1, e2) -> union (freevars e1, freevars e2);;
    

let testFreeVars = Let ([("x1", Prim( "+", CstI 5, CstI 7)); ("x2", Prim( "*", Var "x1", CstI 2))], (Prim("+", Var "x1", Var "x2")))
