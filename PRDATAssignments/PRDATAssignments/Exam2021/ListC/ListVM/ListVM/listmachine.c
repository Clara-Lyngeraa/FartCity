...
int execcode(word p[], word s[], word iargs[], int iargc, int /* boolean */ trace) {
  ...
  for (;;) {
    ...
    case CREATETABLE: {
      uword size = Untag(s[sp--]); // Untag the size from the stack
      word* tablePtr = allocate(1, size + 1, s, sp); // Allocate memory for table, tag 1 for table
      tablePtr[1] = Tag(size); // Tag and store the size of the table in the first element
      // Initialize elements to 0 (assuming the table is zero-indexed)
      for (uword i = 2; i <= size + 1; ++i) {
          tablePtr[i] = 0; // Set each element to 0
      }
      s[++sp] = (word)tablePtr; // Push the pointer to the table onto the stack
    } break;
    case UPDATETABLE: {
      word value = s[sp--]; // Value to be stored in the table
      uword index = Untag(s[sp--]); // Index where the value should be updated
      word* tablePtr = (word*)s[sp--]; // Pointer to the table
      tablePtr[index + 1] = value; // Update element at the specified index
      s[++sp] = (word)tablePtr; // Push the table pointer back onto the stack
    } break;
    case INDEXTABLE: {
      uword index = Untag(s[sp--]); // Untag the index from the stack
      word* tablePtr = (word*)s[sp--]; // Pointer to the table
      s[++sp] = tablePtr[index + 1]; // Push the value at the specified index onto the stack
    } break;
    case PRINTTABLE: {
      word* tablePtr = (word*)s[sp--]; // Pointer to the table
      uword size = Untag(tablePtr[1]); // Retrieve the size of the table
      printf("[");
      for (uword i = 2; i <= size + 1; ++i) {
          printf("%lld", IsInt(tablePtr[i]) ? Untag(tablePtr[i]) : tablePtr[i]);
          if (i < size + 1) {
              printf(", ");
          }
      }
